			...
			if next_entity not in entities_in_current_sentence:
				if [current_id, next_id] not in existing_relation:
					if [next_id, current_id] not in existing_relation:
						...
						relation_l = relation_l[0] + '\t' + 'CID' + '\t' + relation_l[1] + '\t' + \
									 relation_l[2] + '\t' + left_id + '\t' +\
									 relation_l[-2]+ '\t' + relation_l[-1]
						relation_r = relation_r[1] + '\t' + relation_r[2] + '\t' + right_id \
									 + '\t' + relation_r[-2]+ '\t' + relation_r[-1]
...
def get_relation_sentence(inter_sentence_list):
	...
	for j in xrange(len(article_list)):
		...
		if doc_in not in doc_id:
			print str(j) + '	' + doc_in
			relation_l = 'dontknow' + '\t' + 'CID' + '\t' + 'xx' + '\t' + 'xx' + \
						 '\t' + '-NKB48' + '\t'+ '-xx' + '\t'+ '-xx'
			relation_r = '-xx' + '\t' + '-xx' + '\t' + '-NKB48' + '\t'+ '-xx' + '\t'+ '-xx'
	...
	def judge_relation(relation_lists, true_relation):
		...
		for x in xrange(len(relation_lists)):
			...
			if start1 in start_list and start2 in start_list:
				...
				chem_id = temp[4]
				dis_id = temp[9]
				index_start1 = int(temp[5])
				index_end1 = int(temp[6])
				index_start2 = int(temp[-2])
				index_end2 = int(temp[-1])
				if start1 < start2:
					# if start1==49 and start2==274: print ' if yes'	#nothing
					length = int(index_end1) - int(index_start1) - 2
					start_left = int(start1) - int(index_start1) - 2
					end_left = int(end1) - int(index_start1) - 2
					start_right = int(start2) - int(index_start2) + length
					end_right = int(end2) - int(index_start2) + length
				else:
					# if start1 == 49 and start2 == 274 : print 'else yes'	#yes
					length = int(index_end2) - int(index_start2) - 2
					start_left = int(start2) - int(index_start2) - 2
					end_left = int(end2) - int(index_start2) - 2
					start_right = int(start1) - int(index_start1) + length
					end_right = int(end1) - int(index_start1) + length
				relation = str(temp[0]) + '\t' + 'CID' + '\t' + str(start_left) + '\t' + \
							str(end_left) + '\t' + str(chem_id) + '\t' + str(start_right) + '\t' + \
							str(end_right) + '\t' + str(dis_id)
				if [chem_id, dis_id] in true_relations:
					good_relation.append('Relation: ' + relation + '\t' + '1' + '\n')
				else:
					good_relation.append('Relation: ' + relation + '\t' + '0' + '\n')
			else:
				continue
